...
{
	public static class FlipVerticalDecorator
	{
		public static async Task FlipVerticalAsync(this IColorMatrix sourceColorMatrix)
		{
			...
			for (uint row = 0; row < sourceColorMatrix.Height; row++)
			{
				...
				{
					await sourceColorMatrix.SetItem(row, column, me.ColorItems[(sourceColorMatrix.Height - 1) - row, column]);
				}
			}
		}
	}
